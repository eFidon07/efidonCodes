  ...
  const submitForm = async () => {
    ...
    try {
      const response = await fetch(
        "https://send-email-api-phi.vercel.app/api/v1/mail/send",
        {
    ...
    <section className="w-full min-h-screen md:h-screen">
      <div className="max-w-screen-2xl w-full mx-auto h-full flex flex-col items-center px-10 pt-28 text-white">
        ...
        <form
          ...
          method="post"
          className="w-full md:w-200 mt-8 flex flex-col items-center gap-y-7 pb-20"
        >
          <TextInput
            label="Your name"
            name="username"
          ...
          <button
            type="submit"
            className={`group mt-4 w-full md:w-100 h-20 bg-[#242424] flex items-center justify-center border border-transparent ${
              formState.loading
                ? "border-lime-500 text-lime-500"
                : "hover:border-lime-500/85 text-lime-500 md:text-white hover:text-lime-500"
